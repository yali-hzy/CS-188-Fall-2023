...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    variables.extend([GHOST0, GHOST1, PAC, OBS0, OBS1])
    ...
    for obs in [OBS0, OBS1]:
        variableDomainsDict[obs] = range(X_RANGE + Y_RANGE - 2 + MAX_NOISE + 1)
    "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        for key in self.keys():
            self[key] /= sum # cannot write self.total() here since modified
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if noisyDistance == None:
            prob = int(ghostPosition == jailPosition)
        else:
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        E_t : Observation_t
        B_t(x) = 
          P(x_t | e_1:t) \propto P(e_t | x_t) * P(x_t | e_1:t-1)
        '''
        pacmanPosition = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()
        for ghostPositionT in self.allPositions:
            prob = self.getObservationProb(observation, pacmanPosition, 
                                           ghostPositionT, jailPosition)
            self.beliefs[ghostPositionT] *= prob
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        E_t : Observation_t
        B'_t(x) = 
          P(x_t | e_1:t-1) = \sum_{x_{t-1}} P(x_t|x_t-1) * B(x_t-1)
        '''
...
class ParticleFilter(InferenceModule):
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            beliefDistribution[particle] += 1 / self.numParticles
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            weight[particle] += self.getObservationProb(observation, pacmanPosition, 
